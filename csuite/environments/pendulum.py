...
# Default reward and start state function.
def sparse_reward(state: State, reward_angle: int = _REWARD_ANGLE) -> float:
...
class Pendulum(base.Environment):
  ...
  def start(self):
    ...
    # Function for equations of motion for the pendulum.
    def pendulum_fn(_, state_array: np.ndarray):
      """Desired function to be integrated to return the next pendulum state.

      Follows the `fun` callable function signature from the inputs for
      SciPy integrate.

      Args:
        state_array: A (2,) NumPy array with angle and velocity components.

      Returns:
        A (2,) NumPy array giving the velocity and acceleration, which will
        return the new angle and velocity of the pendulum once integrated.
      """
      angle, velocity = state_array
      return (
          velocity,
          self._torque - self._params.friction * velocity -
          self._params.gravity * np.sin(angle),
      )

    # Initialize solver for updating pendulum position.
    self._solver = integrate.RK23(
        pendulum_fn,
        t0=0.,
        y0=(self._state.angle, self._state.velocity),
        t_bound=np.inf,
        first_step=self._params.simulation_step_size,
        max_step=self._params.simulation_step_size)

    # Ensure the angle is between 0 and 2*pi.
    self._solver.y[0] = _alias_angle(self._solver.y[0])

    # Clip velocity to max_speed.
    self._solver.y[1] = np.clip(self._solver.y[1], -self._params.max_speed,
  ...
  def step(self, action):
    ...
    # Update state.
    self._state = State(angle=self._solver.y[0], velocity=self._solver.y[1])
    return (
  ...
  def set_state(self, state):
    ...
    Along with setting the environment state, this method also appropriately
    updates the internal ODE solver state. The solver is not completely
    serializable and thus there is a slight, negligable difference in the
    resulting approximations after setting to a previously seen state.
    ...
    # Update solver.
    self._solver.y[0] = state.angle
    self._solver.y[1] = state.velocity
    self._solver.t = 0

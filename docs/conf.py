...
def _recursive_add_annotations_import():
  for path, _, files in os.walk('../csuite/'):
    for file in files:
      if file.endswith('.py'):
...
project = 'CSuite'
copyright = '2022, DeepMind'  # pylint: disable=redefined-builtin
author = 'CSuite Contributors'
...
def linkcode_resolve(domain, info):
  ...
  return 'https://github.com/deepmind/csuite/tree/master/csuite/%s#L%d#L%d' % (
      os.path.relpath(filename, start=os.path.dirname(
          csuite.__file__)), lineno, lineno + len(source) - 1)
